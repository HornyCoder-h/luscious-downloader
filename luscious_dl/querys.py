def album_info_query(album_id: str) -> dict:
    ...
    return {
        ...
        "query": "query AlbumGet($id: ID!) {album {get(id: $id) {... on Album {...AlbumStandard} ... on MutationError {errors {code message}}}}} fragment AlbumStandard on Album {id title created_by number_of_pictures, number_of_animated_pictures}",
        "variables": {
            "id": album_id
        }
    }
...
def user_favorites_query(user_id: str, sorting: str = 'date_newest', page_number: int = 1) -> dict:
    ...
    return {
        "id": 8,
        "operationName": "AlbumList",
        "query": "query AlbumList($input: AlbumListInput!) {album {list(input: $input) {info {...FacetCollectionInfo} items {...AlbumMinimal}}}}fragment FacetCollectionInfo on FacetCollectionInfo {page has_next_page has_previous_page total_items total_pages url_complete}fragment AlbumMinimal on Album {id}",
        "variables": {
            "input": {
                "display": sorting,
                "filters": [

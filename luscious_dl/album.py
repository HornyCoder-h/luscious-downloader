...
class Album:
    ...
    def __init__(self, id_: Union[str, int] = None, title: str = None, author: str = None,
    ...
    def fetch_pictures(self) -> None:
        ...
        while True:
            ...
            self.pictures.extend(
                [picture['url_to_original'] for picture in response['data']['picture']['list']['items']])
            page += 1
...
def search_albums(search_query: str, sorting: str = 'date_trending', page: int = 1, max_pages: int = 1) -> list[Album]:
    ...
    """
    logger.log(5,
               f'Searching albums with keyword: {search_query} | Page: {page} | Max pages: {max_pages} | Sort: {sorting}')
    albums = []
    while True:
